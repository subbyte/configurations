...
function QuakeConsole:display()
    ...
    local client = nil
    local i = 0
    for c in awful.client.iterate(function (c) return c.instance == self.name end, nil, self.screen)
    do
        i = i + 1
        if i == 1 then
            client = c
    ...
    local width, height, x, y, borderwidth
    borderwidth = 1
    width = geom.width - (self.border + borderwidth) * 2
    height = geom.height * self.height
    x = geom.x + self.border
    if self.vert == "top" then y = geom.y + self.border
    elseif self.vert == "bottom" then y = geom.height + geom.y - height - self.border - borderwidth
    else y = geom.y + (geom.height - height)/2 - self.border - borderwidth end

    -- Resize
    awful.client.floating.set(client, true)
    client.border_width = borderwidth
...
function QuakeConsole:new(config)
    ...
    config.height   = config.height   or 0.3            -- height (%)
    config.border   = config.border   or 8              -- border (px)
    config.vert     = config.vert     or "bottom"       -- top, bottom or center

    config.screen   = config.screen                     -- should be inited
